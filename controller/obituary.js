...
export async function createObituary(req, res) {
  const {
    title,group,photo,video,detail,timestamp, created
  } = req.body;

  const obituary = await obitRepository.create(
    title,group,photo,video,detail,timestamp, req.userId, created);
...
export async function updateObit(req, res, next) {
  ...
  const {
    title,group,photo,video,detail,timestamp
  } = req.body;
  ...
  const updatedObit = await obitRepository.update(id, title,group,photo,video,detail,timestamp);
