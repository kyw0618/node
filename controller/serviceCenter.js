...
export async function createAnnouncement(req, res) {
  ...
  const announcement = await centerRepository.saveAnnouncement({title, content, created});
...
export async function createRequest(req, res) {
  ...
  const userRequest = await centerRepository.saveRequest({
    name,
    phone,
    title,
    content,
...
export async function updateUserRequest(req, res) {
  ...
  if(userRequest.userId !== req.userId && config.adminId !== req.userId) {
    return res.status(403).json({"status": "403"});
  }
...
export async function createResponse(req, res) {
  ...
  const userResponse = await centerRepository.saveResponse({
    reqId,
    title,
    content,
    created
